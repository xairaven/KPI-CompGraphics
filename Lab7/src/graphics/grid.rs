...
impl Grid {
    pub fn lines(&mut self, screen: ScreenParams) -> Vec<Line2D> {
        ...
        let width = (
            screen.value_px_to_cm(screen.resolution.width - screen.canvas_center.x + offset.0),
            screen.value_px_to_cm(screen.resolution.width - screen.canvas_center.x - offset.0),
        );

        // Sides of grid: bottom and top
        let height = (
            screen.value_px_to_cm(screen.resolution.height - screen.canvas_center.y - offset.1),
            screen.value_px_to_cm(screen.resolution.height - screen.canvas_center.y + offset.1),
        );
